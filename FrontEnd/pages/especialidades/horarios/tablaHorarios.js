...
import CrearHorarioEspecialidades from './crearhorario'
...
function TablaHorarios({ tokenQuery, idDiaSemana, permiso }) {
  ...
  const [visibleEditarHorario, setVisibleEditarHorario] = useState(false)
  const [visible, setVisible] = useState(false)
  const [rowDataEliminarHorario, setRowDataEliminarHorario] = useState(null)
  const [rowDataEditarHorario, setRowDataEditarHorario] = useState(null)

  const { data: horarios, mutate } = useSWR(
    tokenQuery
      ? [
          GQLHorarios.GET_TABLA_HORARIOS_ESPECIALIDADES,
  ...
  const mutateEliminarHorario = (variables) => {
    return request(
      process.env.NEXT_PUBLIC_URL_BACKEND,
      GQLHorarios.ELIMINAR_HORARIO_ESPECIALIDAD,
      variables,
      { authorization: `Bearer ${tokenQuery}` }
    )
  }
  ...
  const accept = () => {
    mutateEliminarHorario({
      codigoHorarioEspecialidad: parseInt(
        rowDataEliminarHorario.co_horario_especialidad
      )
    }).then(({ eliminarHorarioEspecialidad: { status, message, type } }) => {
      toast.current.show({
        severity: type,
        summary: 'Atención',
        detail: message,
  ...
  function editarHorario(rowData) {
    setVisibleEditarHorario(true)
    setRowDataEditarHorario(rowData)
  }
  ...
  const accionEliminarEditar = (rowData) => {
    return (
      <div>
        ...
        {permiso?.tx_permisos[2] && (
          <Button
            onClick={() => editarHorario(rowData)}
            icon="pi pi-pencil"
  ...
  return (
    <>
      ...
      />
      <CrearHorarioEspecialidades
        visibled={visibleEditarHorario}
        setVisibled={setVisibleEditarHorario}
        tokenQuery={tokenQuery}
        refresTable={mutate}
        rowDataEditar={rowDataEditarHorario}
      />
      <DataTable
        value={horarios?.getTablaHorariosEspecialidades}
        paginator
        autoLayout={true}
        stripedRows={true}
        ...
        emptyMessage="No se ha Encontrado Especialidad en este Horario."
      >
        ...
        <Column
          header="Cantidad de Citas por Turno"
          field="nu_cantidad_citas_diarias"
        />
        <Column header="Acción" body={accionEliminarEditar} />
      </DataTable>
    </>
  )
}
